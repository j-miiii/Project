...
async function seed() {
    try {
        ...
        for (const data of hospitalData) {
            const existing = await queryRunner.query('SELECT * FROM hospitals WHERE name = ?', [data.name]);
            if (existing.length === 0) {
                await queryRunner.query('INSERT INTO hospitals (name, created_at, udpated_at) VALUES (?, NOW(), NOW())', [data.name]);
