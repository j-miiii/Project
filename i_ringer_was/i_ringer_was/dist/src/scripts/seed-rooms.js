...
function generateRoomNumbers(wardName, count, startIndex) {
    const rooms = [];
...
function getWardPrefix(wardName) {
    const prefixMap = {
        '내과병동': 'IM',
        '외과병동': 'GS',
        '산부인과병동': 'OB',
...
async function seed() {
    ...
    `);
        ...
        for (const ward of wards) {
            const roomCount = getRandomNumber(1, 5);
            console.log(`\n${ward.hospital_name} - ${ward.ward_name} (ID: ${ward.id}): ${roomCount}개 병실 생성`);
            const existingWardRooms = await queryRunner.query('SELECT * FROM rooms WHERE ward_id = ?', [ward.id]);
            ...
            roomStartIndex += roomCount;
            for (let i = 0; i < roomCount; i++) {
                const bedCount = getRandomNumber(2, 5);
                const roomName = roomNames[i];
                try {
                    await queryRunner.query('INSERT INTO rooms (ward_id, name, bed_count, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())', [ward.id, roomName, bedCount]);
