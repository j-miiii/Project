...
async function seed() {
  try {
    ...
    for (const hospital of hospitals) {
      ...
      // 해당 병원의 기존 병동 확인
      const hospitalWards = await queryRunner.query(
        'SELECT * FROM wards WHERE hospital_id = ?',
        [hospital.id]
      );
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const existing = await queryRunner.query(
          'SELECT * FROM wards WHERE hospital_id = ? AND name = ?',
          [hospital.id, wardName]
        );

        if (existing.length === 0) {
          await queryRunner.query(
            'INSERT INTO wards (hospital_id, name, created_at, udpated_at) VALUES (?, ?, NOW(), NOW())',
            [hospital.id, wardName]
          );
