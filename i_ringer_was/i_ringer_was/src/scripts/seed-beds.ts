...
// 침상 상태 랜덤 선택 (70% available, 25% occupied, 5% maintenance)
function getRandomStatus(): string {
  const rand = Math.random();
  if (rand < 0.70) return 'available';
  else if (rand < 0.95) return 'occupied';
...
async function seed() {
  try {
    ...
    for (const room of rooms) {
      ...
      // 해당 병실의 기존 침상 확인
      const existingRoomBeds = await queryRunner.query(
      ...
      for (let i = 1; i <= room.bed_count; i++) {
        ...
        try {
          await queryRunner.query(
            'INSERT INTO beds (room_id, bed_number, status, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())',
            [room.room_id, bedNumber, status]
          );
